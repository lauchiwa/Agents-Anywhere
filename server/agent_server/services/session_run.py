...
class SessionRunService:
    ...
    async def stop_task_in_session(
        self,
        session_id: str,
        task_id: str,
        *,
        ...
        try:
            result = await self._manager.request(session.connectorId, "task.stop", params)
        except ConnectorOfflineError as exc:
            raise SessionRunConflictError(str(exc)) from exc
        except ConnectorRpcError as exc:
            raise SessionRunUpstreamError(exc.message or exc.code) from exc
    ...
    async def fork_session_in_session(
        self,
        session_id: str,
        body: Any,
        *,
    ...
    async def tag_session_in_session(
        self,
        session_id: str,
        body: Any,
        *,
        ...
        try:
            result = await self._manager.request(session.connectorId, "session.tag", params)
        except ConnectorOfflineError as exc:
            raise SessionRunConflictError(str(exc)) from exc
        except ConnectorRpcError as exc:
            raise SessionRunUpstreamError(exc.message or exc.code) from exc
