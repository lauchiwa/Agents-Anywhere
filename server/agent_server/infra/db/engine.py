...
async def _ensure_compat_schema_async(conn) -> None:  # noqa: ANN001 - SQLAlchemy connection
    ...
    if not await _column_exists_async(conn, "sessions", "session_meta_json"):
        await conn.execute(text("ALTER TABLE sessions ADD COLUMN session_meta_json TEXT"))
...
def _ensure_compat_schema_sync(conn) -> None:  # noqa: ANN001 - SQLAlchemy connection
    ...
    if not _column_exists_sync(conn, "sessions", "session_meta_json"):
        conn.execute(text("ALTER TABLE sessions ADD COLUMN session_meta_json TEXT"))
